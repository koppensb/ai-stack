...
def verify(workflows, info):
    ...
    for prefix, encoded in workflows.items():
        ...
        for node_id, node in workflow.items():
            ...
            # ComfyUI V3 exposes reference images as a named Autogrow group;
            # API graphs use flattened inputs such as images.image_1.
            for group, definition in list(allowed.items()):
                if definition[0] != 'COMFY_AUTOGROW_V3':
                    continue
                template = definition[1]['template']
                names = template['names']
                entry = next(value for fields in template['input'].values()
                             for value in fields.values())
                required.pop(group, None)
                allowed.pop(group)
                allowed.update({f'{group}.{name}': entry for name in names})
                for name in names[:template.get('min', 0)]:
                    required[f'{group}.{name}'] = entry
            for key in required:
